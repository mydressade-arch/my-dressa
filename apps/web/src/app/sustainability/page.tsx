export default function SustainabilityPage() {
  return (
    <div>
      <div style={{ background:'#064E3B', padding:'80px 64px', textAlign:'center' }}>
        <span className="material-symbols-outlined" style={{ fontSize:48, color:'#9E896A', display:'block', marginBottom:16 }}>eco</span>
        <h1 style={{ fontFamily:"'Playfair Display', serif", fontSize:48, fontWeight:700, color:'#f4f0ef', marginBottom:16 }}>Sustainable Fashion</h1>
        <p style={{ fontSize:16, color:'rgba(244,240,239,0.75)', maxWidth:540, margin:'0 auto' }}>Every rental is a step toward a more sustainable fashion industry.</p>
      </div>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:32, marginBottom:64 }}>
          {[['70%','Less CO₂ vs buying new'],['3x','Longer garment lifespan'],['0','Single-use fashion'],].map(([n,l])=>(
            <div key={l} style={{ textAlign:'center', padding:32, background:'#f7f3f2' }}>
              <p style={{ fontFamily:"'Playfair Display', serif", fontSize:40, fontWeight:700, color:'#064E3B', marginBottom:8 }}>{n}</p>
